...
describes what users get. Pick a format with `-o`, or let dbi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dbi <command> -o table   # aligned columns for reading
dbi <command> -o jsonl   # one JSON object per line, for piping
dbi <command> -o json    # a single JSON array
dbi <command> -o csv     # spreadsheet friendly
dbi <command> -o tsv     # tab-separated
dbi <command> -o url     # just the URL column
dbi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
dbi <command> --fields id,title,url
```
...
```bash
dbi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
dbi <command>            # a table, because this is a terminal
dbi <command> | wc -l    # JSONL, because this is a pipe
```
